...
void lv_test_led_1(void)
{
    ...
    lv_obj_set_pos_scale(led1, 20, 20);
    ...
    style.body.color_main = COLOR_MAKE(0xb5, 0x0f, 0x04);
    style.body.color_gradient = COLOR_MAKE(0x50, 0x07, 0x02);
    ...
    lv_obj_align_scale(led2, led1, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 20);

    /*Turned ON LED*/
    lv_obj_t * led_on = lv_led_create(lv_scr_act(), led2);
    lv_obj_align_scale(led_on, led2, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 20);
    lv_led_on(led_on);

    /*Tuned OFF LED*/
    lv_obj_t * led_off = lv_led_create(lv_scr_act(), led_on);
    lv_obj_align_scale(led_off, led_on, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    lv_led_off(led_off);

    /*Arbitrary brightness*/
    lv_obj_t * led_x = lv_led_create(lv_scr_act(), led_off);
    lv_obj_align_scale(led_x, led_off, LV_ALIGN_OUT_RIGHT_MID, 10, 0);

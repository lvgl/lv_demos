...
{
    tv = lv_tabview_create(lv_scr_act(), LV_DIR_TOP, LV_DPI_DEF / 2);
#if LV_USE_THEME_DEFAULT
    lv_obj_t * tv_btns = lv_tabview_get_tab_btns(tv);
    lv_obj_set_style_pad_left(tv_btns, LV_PART_MAIN, LV_STATE_DEFAULT, LV_HOR_RES / 2);
...
//        lv_obj_set_pos(sw, LV_DPX(10), LV_DPX(10));
    lv_obj_set_style_content_text(sw, LV_PART_MAIN, LV_STATE_DEFAULT, "Dark");
    lv_obj_set_style_content_align(sw, LV_PART_MAIN, LV_STATE_DEFAULT, LV_ALIGN_OUT_RIGHT_MID);
    lv_obj_set_style_content_ofs_x(sw, LV_PART_MAIN, LV_STATE_DEFAULT, 5);
#endif
    ...
    lv_style_set_content_align(&style_panel, LV_ALIGN_OUT_TOP_LEFT);
    lv_style_set_content_ofs_y(&style_panel, - LV_DPX(10));
    ...
    lv_obj_t * m = lv_msgbox_create("Hello", "Message", btns, true);
    lv_obj_t * btnm = lv_msgbox_get_btns(m);
...
static void controls_create(lv_obj_t * parent)
{
    lv_obj_set_layout(parent, &lv_flex_inline);

    lv_obj_t * panel1 = lv_obj_create(parent, NULL);
    lv_obj_set_height(panel1, LV_SIZE_CONTENT);
    lv_obj_set_flex_grow(panel1, 1);
    lv_obj_set_layout(panel1, &lv_flex_even);
    lv_obj_add_style(panel1, LV_PART_MAIN, LV_STATE_DEFAULT, &style_panel);
    lv_obj_set_style_content_text(panel1, LV_PART_MAIN, LV_STATE_DEFAULT, "Basics");
    lv_obj_set_style_pad_row(panel1, LV_PART_MAIN, LV_STATE_DEFAULT, 40);

    lv_obj_t * btn;
    lv_obj_t * label;
    btn = lv_btn_create(panel1, NULL);
    lv_obj_set_size(btn, LV_SIZE_PCT(40), 40);
    lv_obj_set_layout(btn, &lv_flex_center_row);
    label = lv_label_create(btn, NULL);
    lv_label_set_text(label ,"Button 1");

    btn = lv_btn_create(panel1, NULL);
    lv_obj_set_size(btn, LV_SIZE_PCT(40), LV_SIZE_CONTENT);
    lv_obj_add_state(btn, LV_STATE_CHECKED);
    lv_obj_set_layout(btn, &lv_flex_center_row);
    label = lv_label_create(btn, NULL);
    lv_label_set_text(label ,"Button 2");

    lv_switch_create(panel1, NULL);

    lv_checkbox_create(panel1, NULL);

    lv_obj_t * slider = lv_slider_create(panel1, NULL);
    lv_slider_set_value(slider, 40, LV_ANIM_OFF);
//    lv_add_event_cb(slider, slider_event_cb, NULL);
    lv_obj_set_width(slider, LV_SIZE_PCT(100));
...
//    lv_obj_set_style_local_transition_prop_6(slider, LV_SLIDER_PART_KNOB, LV_STATE_DEFAULT, LV_STYLE_VALUE_OPA);
    ...
    slider = lv_slider_create(panel1, NULL);
    lv_obj_set_width(slider, LV_SIZE_PCT(100));
...
//    lv_event_send(slider, LV_EVENT_VALUE_CHANGED, NULL);      /*To refresh the text*/
    ...
    lv_obj_set_layout(panel2, &lv_flex_stacked);
    lv_obj_set_height(panel2, lv_obj_get_height(panel1));
    lv_obj_add_style(panel2, LV_PART_MAIN, LV_STATE_DEFAULT, &style_panel);
    lv_obj_set_flex_grow(panel2, 1);
    lv_obj_set_style_content_text(panel2, LV_PART_MAIN, LV_STATE_DEFAULT, "Text input");

    lv_obj_t * ta = lv_textarea_create(panel2, NULL);
    lv_obj_set_width(ta, LV_SIZE_PCT(100));
    lv_textarea_set_placeholder_text(ta, "E-mail address");
    lv_textarea_set_one_line(ta, true);
    lv_obj_add_event_cb(ta, ta_event_cb, NULL);

    ta = lv_textarea_create(panel2, NULL);
    lv_obj_set_width(ta, LV_SIZE_PCT(100));
...
//    lv_add_event_cb(ta, ta_event_cb, NULL);

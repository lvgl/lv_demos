...
static lv_res_t enable_action(lv_obj_t * btn)
{
    ...
    if(lv_btn_get_state(btn) == LV_BTN_STATE_REL) {
        ...
        lv_obj_set_style(lv_layer_top(NULL), &style_mbox_bg);
        lv_obj_set_click(lv_layer_top(NULL), false);     /*It should be `true` but it'd block the emulated keyboard too*/

        /*Create a message box*/
        lv_obj_t * mbox = lv_mbox_create(lv_layer_top(NULL), NULL);
...
static lv_res_t mbox_action(lv_obj_t * btn, const char * txt)
{
    ...
    lv_obj_set_style(lv_layer_top(NULL), &lv_style_transp);
    lv_obj_set_click(lv_layer_top(NULL), false);

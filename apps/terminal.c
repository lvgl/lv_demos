...
{
    lv_style_get(LV_STYLE_PRETTY, &style_bg);
    style_bg.mcolor = COLOR_MAKE(0x30, 0x30, 0x30);
    style_bg.gcolor = COLOR_MAKE(0x30, 0x30, 0x30);
    style_bg.bcolor = COLOR_WHITE;
    style_bg.ccolor = COLOR_MAKE(0xE0, 0xE0, 0xE0);

    holder = lv_page_create(lv_scr_act(), NULL);
    lv_obj_set_style(holder, &style_bg);
    lv_obj_set_style(lv_page_get_scrl(holder), lv_style_get(LV_STYLE_TRANSP, NULL));
    ...
    lv_page_glue_obj(ta, true);
    lv_btn_set_rel_action(clr_btn, clr_rel_action);
